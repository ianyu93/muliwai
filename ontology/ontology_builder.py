      ...
      if synset is not None:
        ...
        if label in ('MEDICAL_THERAPY', ):
          ...
          if event in hype:
            new_yago_ontology[word] = label
            continue
        if (commodity in hype or vehicle in hype or artifact in hype
            or plant in hype or molecule in hype or compound in hype
            or event in hype or animal in hype or fac in hype or group in hype
            or symptom in hype or location in hype or infectious_agent in hype
            or disease in hype or physical_condition in hype or body_part in hype
            or substance in hype or food in hype or act in hype
            or process in hype):
          continue    
  ...
  def save_cross_lingual_ontology(self, word2ner_file = "word2ner.json", do_small_set=False):
    ...
    if do_small_set:
      ignore_ner_labels = set(['LANGUAGE', 'FOOD', 'LAW', 'DATE', 'EVENT', 'FAC', 'ANIMAL', 'PLANT', \
        'ANAT', 'BIO_CHEM_ENTITY', 'QUANITY', 'DOMAIN_NAME','WORK_OF_ART', 'PRODUCT'])
    else:
      ignore_ner_labels = set(['WORK_OF_ART'])
    yago2ner = [a for a in self.yago2ner if a[1] not in ignore_ner_labels]
    mt5_tok = transformers.AutoTokenizer.from_pretrained("google/mt5-small")
    ...
    if 'GENDER' not in ignore_ner_labels:
    ...
    word2ner = [(a,b) for a, b in word2ner if b not in ignore_ner_labels]
    print ('num of items', len(word2ner))
    block_list = None
    langs =[]
    nerbylangs = {}
